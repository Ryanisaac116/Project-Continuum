                ...
                <div className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8">
                    <div className="flex h-14 sm:h-16 justify-between items-center">
                        <div className="flex items-center gap-4 sm:gap-8">
                            <Link to="/" className="text-lg sm:text-xl font-bold tracking-tight text-gray-900">
                                Continuum
                            </Link>
                            ...
                            <nav className="hidden md:flex gap-6">
                                {navItems.map(item => (
                                    <Link
                                        key={item.path}
                                        to={item.path}
                                        className={`text-sm font-medium transition-colors hover:text-black ${isActive(item.path) ? 'text-black' : 'text-gray-500'
                                            }`}
                                    >
                                        {item.label}
                                    </Link>
                    ...
                    {navItems.map(item => {
                        ...
                        return (
                            <Link
                                key={item.path}
                                to={item.path}
                                className={`flex flex-col items-center justify-center flex-1 py-2 ${active ? 'text-black' : 'text-gray-500'
                                    }`}
                            >
                                <Icon active={active} />
